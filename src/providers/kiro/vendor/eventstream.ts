...
export class ByteQueue {
  ...
  read(length: number): Uint8Array | null {
    ...
    while (written < length) {
      ...
      // `this.length >= length` was checked above, so a chunk always exists here.
      if (!head) break;
  ...
  private byteAt(offset: number): number {
    ...
    for (let i = 0; i < this.chunks.length; i += 1) {
      ...
      if (remaining < available) return chunk[start + remaining] ?? 0;
      remaining -= available;
    }
    return 0;
  }
...
export function crc32(buf: Uint8Array): number {
  ...
  for (let i = 0; i < buf.length; i += 1) {
    crc = (CRC32_TABLE[(crc ^ (buf[i] ?? 0)) & 0xff] ?? 0) ^ (crc >>> 8);
  }
...
export function parseEventFrame(data: Uint8Array, logger: DebugLogger): EventFrame | null {
  try {
    ...
    while (offset < headerEnd) {
      const nameLength = data[offset] ?? 0;
      offset += 1;
      ...
      const valueLength = ((data[offset] ?? 0) << 8) | (data[offset + 1] ?? 0);
      offset += 2;
